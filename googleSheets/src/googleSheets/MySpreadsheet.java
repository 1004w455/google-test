...
public class MySpreadsheet {
  ...
  public static void sheets() throws IOException, ServiceException {
  ...
  public static void names() throws IOException, ServiceException {
    // 먼저 서비스를 만들고 로그인 정보를 입력합니다.
    SpreadsheetService service = new SpreadsheetService("MySpreadsheetIntegration-v1");
    service.setUserCredentials("2014ksg@adwitt.com", "rkdtkdrb123");
    ...
    for (SpreadsheetEntry entry : spreadsheets) {
      ...
      if (entry.getTitle().getPlainText().equals("와이즈버즈 비상연락망_2015.02.10.xlsx")) {
        System.out.println(entry.getTitle().getPlainText());
        List<WorksheetEntry> worksheets = entry.getWorksheets();
        for (WorksheetEntry worksheet : worksheets) {
          if (worksheet.getTitle().getPlainText().equals("03.09")) {
            //System.out.println("1> \t" + worksheet.getTitle().getPlainText());

            URL cellFeedUrl = worksheet.getCellFeedUrl();
            CellFeed cellFeed = service.getFeed(cellFeedUrl, CellFeed.class);
            for (CellEntry cell : cellFeed.getEntries()) {
              if (cell.getTitle().getPlainText().contains("B")) {
                //System.out.println("2> " + cell.getTitle().getPlainText());
                String shortId = cell.getId().substring(cell.getId().lastIndexOf('/') + 1);
